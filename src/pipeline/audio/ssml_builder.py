...
class SSMLBuilder:
    ...
    def __init__(self, voice_configs: Optional[Dict[str, Any]] = None):
        if voice_configs:
    ...
    def build_conversation_ssml(self, scene_data: Dict[str, Any]) -> str:
        """
        conversation 타입 장면을 위한 SSML 생성 (내부 컨텐츠만 반환)
        """
        ...
        # 1. 원어 화자 (화면 1용)
        if native_script:
            ssml_parts.append(f'<voice name="{self.voice_configs["native"]["name"]}" xml:lang="{self.voice_configs["native"]["language"]}">')
        ...
        if learning_script:
            ...
            # 학습어 화자 1
            ssml_parts.append(f'<voice name="{self.voice_configs["learner_1"]["name"]}" xml:lang="{self.voice_configs["learner_1"]["language"]}">')
            ssml_parts.append(f'<prosody rate="medium" pitch="medium">{learning_script}</prosody>')
            ssml_parts.append('</voice>')
            ssml_parts.append('<break time="1s"/>')
            
            # 학습어 화자 2
            ssml_parts.append(f'<voice name="{self.voice_configs["learner_2"]["name"]}" xml:lang="{self.voice_configs["learner_2"]["language"]}">')
            ssml_parts.append(f'<prosody rate="medium" pitch="medium">{learning_script}</prosody>')
            ssml_parts.append('</voice>')
            ssml_parts.append('<break time="1s"/>')
            
            # 학습어 화자 3
            ssml_parts.append(f'<voice name="{self.voice_configs["learner_3"]["name"]}" xml:lang="{self.voice_configs["learner_3"]["language"]}">')
            ssml_parts.append(f'<prosody rate="medium" pitch="medium">{learning_script}</prosody>')
            ssml_parts.append('</voice>')
            ssml_parts.append('<break time="1s"/>')
            
            # 학습어 화자 4 (마지막은 무음 없음)
            ssml_parts.append(f'<voice name="{self.voice_configs["learner_4"]["name"]}" xml:lang="{self.voice_configs["learner_4"]["language"]}">')
            ssml_parts.append(f'<prosody rate="medium" pitch="medium">{learning_script}</prosody>')
            ssml_parts.append('</voice>')
    ...
    def build_intro_ending_ssml(self, scene_data: Dict[str, Any]) -> str:
        """intro/ending 타입 장면을 위한 SSML 생성 (내부 컨텐츠만 반환)"""
    ...
    def build_dialogue_ssml(self, scene_data: Dict[str, Any]) -> str:
        """dialogue 타입 장면을 위한 SSML 생성 (내부 컨텐츠만 반환)"""
        scene_id = scene_data.get("id", "")
        ...
        for i, line in enumerate(script):
            ...
            if text:
                # 화자별 음성 설정
                voice_name, language_code = self._get_speaker_voice(speaker)
                
                ssml_parts.append(f'<voice name="{voice_name}" xml:lang="{language_code}">')
    ...
    def _get_speaker_voice(self, speaker: str) -> tuple:
        """화자별 음성 설정 반환"""
        speaker_mapping = {
            "A": (self.voice_configs["native"]["name"], self.voice_configs["native"]["language"]),
            "B": (self.voice_configs["learner_1"]["name"], self.voice_configs["learner_1"]["language"]),
            "C": (self.voice_configs["learner_2"]["name"], self.voice_configs["learner_2"]["language"]),
            "D": (self.voice_configs["learner_3"]["name"], self.voice_configs["learner_3"]["language"]),
            "E": (self.voice_configs["learner_4"]["name"], self.voice_configs["learner_4"]["language"])
        }
        
        return speaker_mapping.get(speaker, (self.voice_configs["native"]["name"], self.voice_configs["native"]["language"]))
